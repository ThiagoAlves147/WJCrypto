<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link href="/style/app.css" rel="stylesheet">
    <title>WJCrypto | Register</title>
</head>
<body>

    <header class="header-login">
        <h1>WJCrypto</h1>
    </header>
    <br>
    <section class="login-container">
        <div class="login-section">
            <div class="login-title">
                <h1>Register</h1>
            </div>

            <div class="div-login-form">
                <form action="" class="login-form">
                    <label class="login-label">
                        <input type="text" placeholder="Name" class="login-input">
                    </label>

                    <label class="login-label">
                        <input type="text" placeholder="CPF" class="login-input">
                    </label>

                    <label class="login-label">
                        <input type="text" placeholder="RG" class="login-input">
                    </label>

                    <label class="login-label">
                        <input type="date" placeholder="DataNasc" class="login-input">
                    </label>

                    <label class="login-label">
                        <input type="text" placeholder="Cell" class="login-input">
                    </label>

                    
                    <label class="login-label">
                        <input type="text" placeholder="Adress" class="login-input">
                    </label>

                    <label class="login-label">
                        <input type="password" placeholder="Password" class="login-input">
                    </label>

                    <label class="login-label">
                        <input type="password" placeholder="Confirm password" class="login-input">
                    </label>

                    <div class="login-button" style="width: 100%;">
                        <button type="submit" class="logar">Entrar</button>
                    </div>
                </form>

                <br>
                <div style="text-align: center;">
                    Have an account?
                    <a href="login" class="login-link">Sing in</a>
                </div>

            </div>

        </div>
    </section>

</body>
</html>